...
Начиная с какой-то версии lazygit строже валидирует нотацию клавиш при старте и падает с ошибкой вида:
```
Unrecognized key '<c-R>' for keybinding 'Commits.ResetCherryPick[0]'
```
Причина: `<c-R>` (ctrl + заглавная буква) — устаревшая/неоднозначная запись Ctrl+Shift+R. Актуальная нотация — `<c-s-r>` (или `<ctrl+shift+r>`). Она валидна и на старых, и на новых версиях lazygit, так что это универсальный фикс, а не даунгрейд.

В файле сейчас (строка ~197):
```yaml
resetCherryPick: "<c-s-r>"
```
Если на другой машине (например, рабочей) стоит lazygit, который тоже начал ругаться на `<c-R>` — накатить тот же фикс: `<c-s-r>` вместо `<c-R>`.
